    ...
    while (outputIndex < outputSampleCount) {
        ...
        while (mBuffer.frameCount == 0 && inFrameCount > 0) {
            ...
            if (mBuffer.raw == NULL) {
                ...
                AudioDump::threadDump(fileName,out ,outFrameCount* mChannelCount*sizeof(TI),gaf_resampler_propty,
                ...
                char fileName[SIZE];
                sprintf(fileName,"%s_%p.pcm",gaf_resampler_in_pcm, provider);
                AudioDump::threadDump(fileName,mBuffer.raw ,mBuffer.frameCount* mChannelCount*sizeof(TI),gaf_resampler_propty,
                   mInFormat, mInSampleRate, mChannelCount);
            }
#endif
#endif
