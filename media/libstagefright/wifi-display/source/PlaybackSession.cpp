...
status_t WifiDisplaySource::PlaybackSession::addVideoSource(
    ...
    ///M: @{ if hdcp is enable, then skip config the buffer count according encoder inputbuffer
    char val1[PROPERTY_VALUE_MAX],val2[PROPERTY_VALUE_MAX];
    if ( !((property_get("ro.mtk_sec_video_full_support", val1, NULL) && (!strcmp(val1, "1")))&&
            (property_get("media.wfd.hdcp", val2, NULL) && (!strcmp(val2, "1")))) ) {
