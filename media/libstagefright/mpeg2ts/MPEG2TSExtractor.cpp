...
#ifdef MTK_AOSP_ENHANCEMENT
int32_t findSyncCode(const void *data, size_t size,size_t kFillPacketSize) {
    uint32_t i = 0;
    for (i = 0; i < size - kFillPacketSize; i++) {
        if (((uint8_t *) data)[i] == 0x47u &&
            (((uint8_t*)data)[kFillPacketSize + i]==0x47u))
            return i;
    }
    return -1;
}
...
*/
    ...
    i4PidNum = mParser->parsedPIDSize();
    puStrmPidInfo = new unsigned[i4PidNum];
    for (i4PidNumIdx = 0; i4PidNumIdx < i4PidNum; i4PidNumIdx ++){
       puStrmPidInfo[i4PidNumIdx] = mParser->getParsedPID(i4PidNumIdx);
    }
...
status_t MPEG2TSExtractor::feedMore() {
    ...
    status_t retv = OK;
    ssize_t n = mDataSource->readAt(mOffset, packet, kFillPacketSize * 2);
    ...
    if (kFillPacketSize == kM2TSPacketSize) {
        int32_t syncOff=0;
        syncOff = findSyncCode(packet+4, n-4, kFillPacketSize);
        if(syncOff >=0) {
            mOffset -= n;
            mOffset+=syncOff;
        }else{
            return OK;//not find sync word this time
        }
        n = mDataSource->readAt(mOffset, packet, kFillPacketSize);
        mOffset += n;
        ...
        if (retv == BAD_VALUE) {
            int32_t syncOff = 0;
            syncOff = findSyncCode(packet + 4, kFillPacketSize - 4,kFillPacketSize);
            if (syncOff >= 0) {
                mOffset -= n;
                mOffset += syncOff;
            }
        ...
        int32_t syncOff=0;
        syncOff = findSyncCode(packet, n, kFillPacketSize);
        if(syncOff >=0) {
            mOffset -= n;
            mOffset+=syncOff;
        }else{
        ...
        if (retv == BAD_VALUE) {
            int32_t syncOff = 0;
            syncOff = findSyncCode(packet, kFillPacketSize,kFillPacketSize);
            if (syncOff >= 0) {
                mOffset -= n;
                mOffset += syncOff;
            }
            ...
            if (retv != OK){ALOGE("[TS_ERROR] retv:%d mOffset:%lld",retv,(long long)mOffset);}
            return retv;
        }
    }
#else //#if !defined(ANDROID_DEFAULT_CODE) && (defined(SUPPORT_M2TS))
