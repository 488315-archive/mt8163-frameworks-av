...
prepareBitmapBuffer()
{
    ...
    for (int i=0;i < DVB_TMP_FILE_COUNT; i++)
    {
        ...
        mFd[i] = open(filepath, O_CREAT | O_RDWR | O_TRUNC, 0777);
        ...
        ALOGD("[MY] --- start, open fd = %d", mFd[i]);
        if(fchown(mFd[i],1013,1013) < 0) {
            ALOGE("fchown fd:[%s], errno=%d:%s.", filepath, errno, strerror(errno));
        }
        fchmod(mFd[i],0777);
#if 1
...
INT32 DVBParser::paintPage()
{
    ...
    while(DVBR_OK == mCurrPage->getRegionInfo(ui2_rgn_idx,&t_region_info))
    {
        ...
        for (unsigned int y = 0; y < t_region_info.ui4_height;y++)
        {
#if DVB_PAINT_BITMAPDATA_DIRECTLY
            UINT32*     pui4_pos = (UINT32*)((char*)ptrBitmapData + (y + t_region_info.i4_top) * ui4RowBytes + ((t_region_info.i4_left) << 2));
#endif
            for (unsigned int x = 0; x < t_region_info.ui4_width;pui4_pos++,x++)
            {
