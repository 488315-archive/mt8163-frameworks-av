...
void NuPlayer::Renderer::syncQueuesDone_l() {
    ...
    if (!isSyncQueues()) {
        ...
        if(mIsDoSelectTrack == true && mVideoQueue.empty()){
            mIsDoSelectTrack = false;
    #endif
        return;
    #if defined(MTK_AOSP_ENHANCEMENT) && defined(MTK_SUBTITLE_SUPPORT)
        }
...
#ifdef MTK_AOSP_ENHANCEMENT
    /* for ALPS02065697:
    else if (mAllAudioIsDroppedInDecoder) {
        MM_LOGI("mAudioSink->start() - all audio dropped in Decoder");
        mAudioSink->start();
    }
    */
#endif
...
#if defined(MTK_AOSP_ENHANCEMENT) && defined(MTK_SUBTITLE_SUPPORT)
void NuPlayer::Renderer::isDoSelectTrack(){
   mIsDoSelectTrack = true;
}
#endif
